...
function renderCalendar() {
    ...
    const statusBadgeClass = (status) => {
        ...
        const m = {
            "진료 완료": "bg-green-100 text-green-800",
            "진료 중": "bg-neutral-100 text-neutral-700",
            "진료 대기": "bg-yellow-100 text-yellow-800",
            "예약 확정": "bg-indigo-100 text-indigo-800",
            "예약 완료": "bg-blue-100 text-blue-800",
        };
        return m[status] || "bg-neutral-100 text-neutral-700";
    };
    ...
    const cellHTML = (dateObj, isDim) => {
        ...
        // 화면에서 셀 높이가 한정적이므로 최대 표시 개수 제한 (3개)
        const maxShow = 3;
        const apptItems = appts
            .slice(0, maxShow)
            .map(
                (a) =>
                    `<div class="text-xs bg-neutral-100 px-2 py-1 rounded flex items-center justify-between">
        <div class="truncate">
          <span class="text-[11px] font-medium">${a.time}</span>
          <span class="ml-1 truncate">${a.name}</span>
        </div>
        <div class="ml-2 flex-shrink-0">
          ${
              a.status
                  ? `<span class="px-2 py-0.5 rounded text-[10px] ${statusBadgeClass(a.status)}">${a.status}</span>`
                  : ""
          }
        </div>
       </div>`
        ...
        const more =
            appts.length > maxShow
                ? `<div class="text-[10px] text-neutral-500 mt-1">+${appts.length - maxShow} more</div>`
                : "";

        // 셀 내부 overflow를 숨겨서 콘텐츠가 셀을 벗어나지 않도록 함
        return `
    <div class="h-32 p-2 ${border} align-top cursor-pointer overflow-hidden flex flex-col justify-start${bgSel}" data-date="${dateStr}">
    <div class="${dimCls} mb-1">${dateObj.getDate()}</div>
    <div class="mt-1 space-y-1 max-h-20 overflow-hidden">${apptItems}${more}</div>
    </div>
  `;
    };
        ...
        ? list
            ...
            <div class="bg-white p-4 rounded-lg border border-neutral-200 cursor-pointer" onclick="location.href='detailAppointment.html'">
                <div class="flex items-center justify-between mb-2">
                    ...
                    ${
                        a.status
                            ? `<span class="px-2 py-1 rounded text-xs ${statusBadgeClass(a.status)}">${a.status}</span>`
                            : ""
                    }
