...
const patients = {
    김환자: { time: "09:00", dept: "내과", symptom: "감기", birth: "85.03.12", doctor: "김의사", status: "진료 완료" },
    이환자: {
        time: "09:30",
        dept: "정형외과",
        symptom: "무릎 통증",
    ...
    정환자: { time: "11:00", dept: "소아과", symptom: "열", birth: "15.06.09", doctor: "정의사", status: "예약 완료" },
    오환자: { time: "11:30", dept: "내과", symptom: "두통", birth: "99.09.30", doctor: "김의사", status: "진료 대기" },
    윤환자: {
        time: "12:00",
        dept: "정신건강의학과",
        symptom: "불안",
    ...
    임환자: { time: "13:00", dept: "치과", symptom: "충치", birth: "02.12.02", doctor: "임의사", status: "진료 완료" },
    장환자: {
        time: "13:30",
        dept: "정형외과",
        symptom: "허리 통증",
        ...
        <div class="bg-white p-4 rounded-lg border border-neutral-200">
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-2">
                    ${
                        detail.status
                            ? `<span class="px-2 py-1 rounded text-xs ${statusBadgeClass(detail.status)}">${
                                  detail.status
                              }</span>`
                            : ""
                    }
    ...
    times.forEach((t) => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (patient) {
                ...
                if (i === assignedDay) {
                    html += `
                        <div class="text-xs bg-neutral-100 px-2 py-1 rounded cursor-pointer flex items-center justify-between" onclick="showDetail('${
                            patient[0]
                        }')">
                            <div>
                            ...
                            ${
                                patient[1].status
                                    ? `<span class="px-2 py-0.5 rounded text-[10px] ${statusBadgeClass(
                                          patient[1].status
                                      )}">${patient[1].status}</span>`
                                    : ""
                            }
                        </div>
                    `;
